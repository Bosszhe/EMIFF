...
from .dair_kitti_data_utils import get_kitti_image_info
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        # Trv2c = calib['veh_Tr_velo_to_cam']
        P2 = calib['P2']
        if remove_outside:
...
def create_kitti_info_file(data_path,
    ...
    train_img_ids = _read_imageset_file(str(imageset_folder /'veh'/ 'train.txt'))
    val_img_ids = _read_imageset_file(str(imageset_folder /'veh' / 'val.txt'))
    # test_img_ids = _read_imageset_file(str(imageset_folder /'veh'/ 'test.txt'))
    inf_train_img_ids = _read_imageset_file(str(imageset_folder /'inf'/ 'train.txt'))
    inf_val_img_ids = _read_imageset_file(str(imageset_folder /'inf'/ 'val.txt'))
    
    # from IPython import embed
    # embed(header='create_kitti_info_file')
    ...
    kitti_infos_train = get_kitti_image_info(
        ...
        inf_image_ids=inf_train_img_ids,
