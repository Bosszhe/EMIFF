...
class MS_NaiveCompressor(nn.Module):

    def __init__(self, input_dim, c_compress_ratio, s_compress_ratio):
        super(MS_NaiveCompressor, self).__init__()

        # from IPython import embed
        # embed(header='MSC')

        assert s_compress_ratio<=3 & s_compress_ratio >=0
        s_compress_ratio0 = max(s_compress_ratio,0)
        s_compress_ratio1 = max(s_compress_ratio-1,0)
        s_compress_ratio2 = max(s_compress_ratio-2,0)
        s_compress_ratio3 = max(s_compress_ratio-3,0)



        self.compressor0 = NaiveCompressor_UNet(input_dim,c_compress_ratio,s_compress_ratio0)
        self.compressor1 = NaiveCompressor_UNet(input_dim,c_compress_ratio,s_compress_ratio1)
        self.compressor2 = NaiveCompressor_UNet(input_dim,c_compress_ratio,s_compress_ratio2)
        self.compressor3 = NaiveCompressor_UNet(input_dim,c_compress_ratio,s_compress_ratio3)

    def forward(self, x):
        assert x.__len__() == 4

        # from IPython import embed
        # embed(header='MS_NaiveCompressor.forward')

        x0 = self.compressor0(x[0])
        x1 = self.compressor1(x[1])
        x2 = self.compressor2(x[2])
        x3 = self.compressor3(x[3])
...
class VICFuser_Voxel_MS_Attention_C(BaseDetector):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 neck_dcn=None):
        print('VICFuser_Voxel_MS_Attention_C_0110.__init__')
        ...
        self.inf_compressor = MS_NaiveCompressor(self.img_feat_channels, self.c_compress_ratio, self.s_compress_ratio)

        # self.ms_conv_v = MultiScaleConv(neck.in_channels,neck.out_channels)
        # self.ms_conv_i = MultiScaleConv(neck.in_channels,neck.out_channels)
