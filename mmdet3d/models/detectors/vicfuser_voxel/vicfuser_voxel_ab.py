...
class VICFuser_Voxel_AB(BaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        print('VICFuser_Voxel_AB.__init__')
    ...
    def extract_feat(self, img, img_metas, pipe):
        ...
        # embed(header='VICFuser_Voxel_AB.extract_feat')
        ...
        x_v_feature = x_v.cpu().numpy()
        pipe.send("x_v_feature", x_v_feature)
    ...
    def forward_train(self, img, img_metas, gt_bboxes_3d, gt_labels_3d,
        ...
        """
        from IPython import embed
        embed(header='VICFuser_Voxel_AB.forward_train')
    ...
    def forward_test(self, img, img_metas, **kwargs):
        ...
        # from IPython import embed
        # embed(header='VICFuser_Voxel_AB.forward_test')
        
        # not supporting aug_test for now
        return self.simple_test(img, img_metas, pipe=kwargs['pipe'])

    def simple_test(self, img, img_metas, pipe):
        ...
        x = self.extract_feat(img, img_metas, pipe)
        x = self.bbox_head(x)
